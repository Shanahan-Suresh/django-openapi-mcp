...
async def _execute(spec: ToolSpec, arguments: dict[str, Any], cfg: dict[str, Any]) -> str:
    """Map tool arguments onto the HTTP request and return a text result."""
    path = spec.path
    query: dict[str, Any] = {}
    ...
    async with httpx.AsyncClient(timeout=cfg["TIMEOUT"]) as client:
        resp = await client.request(
            spec.method, url, params=query, headers=headers, follow_redirects=True
        )
    ...
    try:
        ...
        pass
    return f"HTTP {resp.status_code} {spec.method} {url}\n\n{body}"
